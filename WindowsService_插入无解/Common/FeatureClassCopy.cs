...
namespace WindowsService.Common
{
    ...
    public class FeatureClassCopy
    {
        ...
        //更新
        public static void updateFeatureClass(string _FeatureDatasetNameSource, List<string> _FeatureClassNameSourceList, string _FeatureDatasetNameTarget, List<string> _FeatureClassNameTargetList, List<string> SourSet, List<string> TargetSet)
        {
            IWorkspace pWSSource = GetSDEWorkspace(SourSet[0], SourSet[1], SourSet[2], SourSet[3], SourSet[4], SourSet[5]);
            IWorkspace2 pWStarget = (IWorkspace2)GetSDEWorkspace(TargetSet[0], TargetSet[1], TargetSet[2], TargetSet[3], TargetSet[4], TargetSet[5]);
            updateFeatureClass(_FeatureDatasetNameSource, _FeatureClassNameSourceList, _FeatureDatasetNameTarget, _FeatureClassNameTargetList, pWSSource, pWStarget);
        }

        //备份
        public static void backupFeatureDataset(string _FDatasetNameSou, List<string> _FClassNameSouList, string xmlpath, List<string> SourSet, List<string> TargetSet)
        {

            IWorkspace pWSSource = GetSDEWorkspace(SourSet[0], SourSet[1], SourSet[2], SourSet[3], SourSet[4], SourSet[5]);
            IWorkspace2 pWStarget = (IWorkspace2)GetSDEWorkspace(TargetSet[0], TargetSet[1], TargetSet[2], TargetSet[3], TargetSet[4], TargetSet[5]);
            backupFeatureDataset(_FDatasetNameSou, _FClassNameSouList, xmlpath, pWSSource, pWStarget);
        }
        ...
        public static void backupFeatureDataset(string _FDatasetNameSou, List<string> _FClassNameSouList, string xmlpath, IWorkspace sourceW, IWorkspace2 targetW)
        {
            IWorkspace pWSSource = sourceW;
            IWorkspace2 pWStarget = targetW;
            if(pWSSource!=null&&pWStarget!=null)
            {
                ...
                {                    
                    ...
                    //for (int i = 0; i < _FClassNameSouList.Count;i++ )
                    {
                        //获取数据源图层                        
                ...
                else  //FeatureDatatset不存在
                {
                    //创建FeatureDataSet
                    IFeatureWorkspace pFWSSource = (IFeatureWorkspace)pWSSource;
                    IFeatureDataset pFDatasetSour = pFWSSource.OpenFeatureDataset(_FDatasetNameSou);
                    IGeoDataset geodataset = (IGeoDataset)pFDatasetSour;
                    ISpatialReference fdsSR = geodataset.SpatialReference;// createSpatialReference(false, 2365);
        ...
        {            
            ...
            ESRI.ArcGIS.Geodatabase.IQueryFilter pQueryFilter = new ESRI.ArcGIS.Geodatabase.QueryFilter();
            //检查是否有记录
                ...
                try
                {
                    ...
                    while ((inputpFeature = pFtCursor.NextFeature()) != null)
                    {
                        ...
                        for (int i = 0; i < inputpFeature.Fields.FieldCount; i++)
                        {
                            ...
                            if ((field.Type != esriFieldType.esriFieldTypeGeometry) && (field.Type != esriFieldType.esriFieldTypeOID) && field.Editable && field.Name.ToLower().Contains("shape") != true)//TargetFtClass.LengthField && field != TargetFtClass.AreaField)
                            {
        ...
        private static ESRI.ArcGIS.Geometry.ISpatialReference createSpatialReference(System.Boolean coorditateType, System.Int32 spatialRefEnum)
        {
            ...
            ISpatialReferenceFactory3 pSpaRefFactory = new SpatialReferenceEnvironment() as ISpatialReferenceFactory3;
            if (coorditateType)
            {
                //创建地理坐标系对象
        ...
        private static IWorkspace GetSDEWorkspace(string sServerName, string sInstancePort, string sUserName, string sPassword, string database, string sVersionName)
        {
            ...
            try
            {                
                return workspaceFactory.Open(set, 0);
            }
            catch
            {
                return null;
